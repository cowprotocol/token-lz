...
The OFTAdapter locks tokens on the origin chain, while OFT contracts on other chains mint/burn corresponding tokens to maintain cross-chain fungibility. There can be only one OFTAdapter for a multi-chain OFT token deployment.
...
3. (optional) Set deployer credentials in `.env`:
```
...
4. (optional) Fund the deployer address with native tokens on target chains
...
**NOTE:** you may see a warning about `PRIVATE_KEY` environment variable not being read if you skipped step (3) above. This is normal.

## Deployment

### 1. Configure Token Address

**NOTE:** In general this step can be skipped because the change has already been applied to the repo. This information is left here in case you are deploying a test/forked token.
...
The output of the last 2 commands can then be pasted into an editor without word wrap for comparison.

All bytes should be the same except for in a couple places--where the `immutable` variable for the lzEndpoint is specified.

#### Etherscan Verification

The contracts can be verified on Etherscan by manually submitting the `solcInputs.json` file exported as part of the deployments file:
...
Its handy to use the CLI to send tokens once they are deployed:
